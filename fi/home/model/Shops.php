...
class Shops extends Base
{
    ...
    public function getByView($id)
    {
        ...
        foreach ($catshops as $key => $v) {
            if (isset($catshopMaps[$v['catId']])) {
                $catshopNames[] = $catshopMaps[$v['catId']];
            }
     ...
     */
    public function getShopInfoAndAddress($userId)
    {
        $shopInfo = $this->where(["userId" => $userId, "dataFlag" => 1])->find();
        $location = array();
        if ($shopInfo) {
            $location = (new Areas)->getLocationByAreaIdPath($shopInfo['areaIdPath']);
        }
