	...
	grid = $("#maingrid").ligerGrid({
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            var h = "";
	            if(FI.GRANT.HYGL_02)h += "<a href='"+FI.U('admin/Users/toEdit','id='+rowdata['userId'])+"'>修改</a> ";
	            if(FI.GRANT.HYGL_03)h += "<a href='javascript:toDel(" + rowdata['userId'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
    ...
    $('#userForm').validator({
            ...
            rules: {
                ...
                myRemote: function(element){
                    return $.post(FI.U('admin/users/checkLoginKey'),{'loginName':element.value,'userId':$('#userId').val()},function(data,textStatus){});
                }
            },
            fields: {
                loginName: {
          ...
          valid: function(form){
            ...
            var loading = FI.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(FI.U('admin/Users/'+((params.userId==0)?"add":"edit")),params,function(data,textStatus){
              layer.close(loading);
              var json = FI.toAdminJson(data);
              if(json.status=='1'){
                  FI.msg("操作成功",{icon:1});
