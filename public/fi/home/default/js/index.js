...
FI.slides = function(){
	var slide = $('#fi-slide'), li = slide.find("li");
	var slidecontrols = $('.fi-slide-controls').eq(0), 
...
/*楼层右侧滚动广告*/
function floorAds(i){
	var slide = $('#fi-floor-slide-'+i), li = slide.find("li");
	var slidecontrols = $('#fi-floor-slide-controls-'+i), 
...
$(function(){
	//执行楼层右侧广告js
	var fRAds = $(this).find("div[id^='fi-floor-slide-controls-']").length;
...
function fBGoods(id){
	$('#styleMain'+id).bxCarousel({
		...
		auto: 0, 
		controls: true,
		prev_image: conf.ROOT+'/fi/home/default/img/btn_slide_left.png',
    	next_image: conf.ROOT+'/fi/home/default/img/btn_slide_right.png',
		margin: 10,
		auto_hover: true
	});
}
...
function leftNav(){
	//内容距离左边空白处宽度
	var containerW = $('.fi-container').offset().left;
	left = containerW-50;
...
//内容高度
var containerH = parseInt($('.fi-container').css('height'));
