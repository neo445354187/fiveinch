...
$.fn.FIGridTree = function(options){
	...
	var defaults = {id:'fi_'+radomId,tbodyId:'fi_tbody_'+radomId,width:'100%',headerRowHeight:28,rowHeight:28,nodeNum:0,params:{},level:0}; 
	var opts = $.extend(defaults, options);
	var jqObj = $(this);
	var treeKeyId;
	var createTBHead = function(){
		var head = [],htmp,rowAlign,rowWidth;
		head.push('<table class="l-grid-header-table fi-grid-tree" cellspacing="0" cellpadding="0" width="'+opts.width+'"><tbody id="'+opts.tbodyId+'"><tr height="'+opts.headerRowHeight+'" class="l-grid-hd-row fi-grid-tree-hd">');
		...
		$.getJSON(_opts.url,_opts.params,function(data,textStatus){
			if(data.Rows){
			    ...
			    for(var i=0;i<data.length;i++){
					 ...
					 opts.nodeNum++;
					 body = [];
					 body.push('<tr id="'+nodeId+'" height="'+opts.rowHeight+'" class="l-grid-row fi-grid-tree-row j-'+ttmp[treeKeyId]+'" dataid="'+ttmp[treeKeyId]+'" pdataid="'+nodeId+'" lv="'+_opts.level+'">');
					 ...
					 for(var j=0;j<opts.columns.length;j++){
						 ...
						 if(htmp.id){
							 ...
							 prefix+='<div class="l-grid-tree-space l-grid-tree-link l-grid-tree-link-close fi-tree-img" dataid="'+ttmp[treeKeyId]+'" pdataid="'+nodeId+'" lv="'+_opts.level+'"></div>';
					 ...
					 lastNodeId = nodeId;
					 $('#'+nodeId+" .fi-tree-img").click(function(){
	...
	var changeRowColor = function(){
		var even = false;
		$('.fi-grid-tree-row').each(function(){
