        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',width: 150,isSort: false,
	        	render: function (rowdata){
		            var h = "";
			        if(FI.GRANT.SPFL_01)h += "<a href='javascript:toEdit("+rowdata["catId"]+",0)'>新增子分类</a> ";
		            if(FI.GRANT.SPFL_02)h += "<a href='javascript:toEdit("+rowdata["parentId"]+","+rowdata["catId"]+")'>修改</a> ";
		            if(FI.GRANT.SPFL_03)h += "<a href='javascript:toDel("+rowdata["parentId"]+","+rowdata["catId"]+")'>删除</a> "; 
		            return h;
	        	}}
        ]
    });
...
function toggleIsFloor(obj,id){
	...
	$.post(FI.U('admin/goodscats/editiIsFloor'),{id:id,isFloor:v},function(data,textStatus){
		  layer.close(loading);
...
function toggleIsShow(obj,id){
    ...
    var loading = FI.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    var v = ($(obj).attr('v')=='1')?0:1;
    $.post(FI.U('admin/goodscats/editiIsShow'),{id:id,isShow:v},function(data,textStatus){
		layer.close(loading);
		var json = FI.toAdminJson(data);
		if(json.status=='1'){
			 FI.msg(json.msg,{icon:1});
...
function toEdit(pid,id){
	...
	if(id>0){
		...
		FI.setValues({parentId:pid,catName:'',isShow:1,isFloor:0,catSort:0});
		editsBox(id);
	}
}
