...
$(function(){
	...
	$('#menuTree').height(FI.pageHeight()-36);
	var setting = {
	      view: {
	           selectedMulti: false,
	      ...
	      async: {
	           enable: true,
	           url:FI.U('admin/menus/listQuery'),
	...
	grid = $("#maingrid").ligerGrid({
		url:FI.U('admin/privileges/listQuery'),
		pageSize:100,
		pageSizeOptions:[100],
		height:'99%',
        width:'100%',
...
function onRightClick(event, treeId, treeNode) {
	...
	$('#'+treeNode.tId).bind('contextmenu',function(e){
		 var items = [];
		 if(FI.GRANT.CDGL_01)items.push({
			 icon : 'add',
             text : '新增菜单',
		 ...
		 treeNode = zTree.getSelectedNodes()[0];
		 if(treeNode.id>0){
			 if(FI.GRANT.CDGL_02)items.push({
                 icon  : 'pencil',
                 text  : '编辑菜单',
                 click : function(parent, menu) {
                  	treeNode = zTree.getSelectedNodes()[0];
			 ...
			 if(FI.GRANT.CDGL_03)items.push({
                 icon  : 'remove',
                 text  : '删除菜单',
                 click : function(parent, menu) {
                  	treeNode = zTree.getSelectedNodes()[0];
                  	layer.confirm('您确定要删除该菜单['+treeNode.name+']吗？', {btn: ['确定','取消']}, function(){
                  	    var loading = FI.msg('正在提交请求，请稍后...', {icon: 16,time:60000});
                  	    $.post(FI.U('admin/menus/del'),{id:treeNode.id},function(data,textStatus){
                  		     layer.close(loading);
...
function toDel(id){
	var box = FI.confirm({content:"您确定要删除该权限吗?",yes:function(){
	           var loading = FI.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(FI.U('admin/privileges/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
	           			  var json = FI.toAdminJson(data);
	           			  if(json.status=='1'){
	           			    	FI.msg("操作成功",{icon:1});
...
function checkPrivilegeCode(obj){
	...
	var loading = FI.msg('正在检测代码是否存在，请稍后...', {icon: 16,time:60000});
	$.post(FI.U('admin/privileges/checkPrivilegeCode'),{code:obj.value},function(data,textStatus){
		layer.close(loading);
		var json = FI.toAdminJson(data);
		if(json.status!='1'){
			FI.msg(json.msg,{icon:2});
