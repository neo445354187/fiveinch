...
class Orders extends Base{
	...
	public function submit(){
		...
		if(empty($carts['carts']))return FIReturn("请选择要购买的商品");
		//检测地址是否有效
	...
	public function cancel(){
		...
		if(empty($reasonData))return FIReturn("无效的取消原因");
		if(!empty($order)){
			Db::startTrans();
		    try{
	...
	public function reject(){
		...
		if(empty($reasonData))return FIReturn("无效的拒收原因");
		if($reason==10000 && $content=='')return FIReturn("请输入拒收原因");
		if(!empty($order)){
			Db::startTrans();
		    try{
	...
	public function editOrderMoney(){
		...
		$userId = (int)session('FI_USER.userId');
		$shopId = (int)session('FI_USER.shopId');
		if($orderMoney<0)return FIReturn("订单价格不能小于0");
		Db::startTrans();
		try{
	...
	public function confer(){
		...
		if($status==0 && $content=='')return FIReturn("请输入不同意原因");
		if(!empty($order)){
			Db::startTrans();
		    try{
