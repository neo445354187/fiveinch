function login(typ){
	...
	$.post(FI.U('home/users/checkLogin'),params,function(data,textStatus){
		...
		if(json.status=='1'){
			if(typ==2){
				location.href=FI.U('home/shops/index');
			}else if(typ==1){
				location.href=FI.U('home/users/index');
			}else{
...
function showProtocol(){
	layer.open({
	    ...
	    content: [FI.U('home/users/protocol')],
	    btn: ['同意并注册'],
...
function getVerifyCode(){
	...
	if(window.conf.SMS_VERFY=='1'){
		var html = [];
			html.push('<table class="fi-smsverfy"><tr><td width="80" align="right">');
			html.push('验证码：</td><td><input type="text" id="smsVerfyl" size="12" class="fi-text" maxLength="8">');
			html.push('<img style="vertical-align:middle;cursor:pointer;height:39px;" class="verifyImg" src="'+FI.DOMAIN+'/fi/Home/default/images/clickForVerify.png" title="刷新验证码" onclick="javascript:FI.getVerify(\'.verifyImg\')"/>');
...
function getPhoneVerifyCode(params){
	...
	$.post(FI.U('home/users/getPhoneVerifyCode'),params,function(data,textStatus){
		var json = FI.toJson(data);
		if(json.status!=1){
			FI.msg(json.msg, {icon: 5});
			time = 0;
...
function initRegist(){
	$('#reg_form').validator({
	    ...
	    valid: function(form){
	        ...
	        me.holdSubmit();
	        var params = FI.getParams('.fi_ipt');
	        $("#reg_butt").css('color', '#999').text('正在提交..');
	        $.post(FI.U('home/users/toRegist'),params,function(data,textStatus){
	    		var json = FI.toJson(data);
	    		if(json.status>0){
	    			FI.msg('注册成功，正在跳转登录!', {icon: 6}, function(){
	    				location.href=FI.U('home/users/index');
	       			});
	    		}else{
	    			me.holdSubmit(false);
