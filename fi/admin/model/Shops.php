...
class Shops extends Base{
	...
	public function add(){
		...
		if($userId==0){
			...
			$ck = FICheckLoginKey($user['loginName']);
        ...
        if($goodsCatIds=='')return FIReturn('请选择经营范围');
        Db::startTrans();
        try{
        	//如果是游客的话就先新增会员资料
        	if($userId==0){
	        ...
	        FIUnset($data,'shopId,dataFlag,isSelf');
	        ...
	        if($userId>0){
	        	...
	        	$shopId = $this->shopId;
	        	//启用上传图片
				FIUseImages(1, $shopId, $data['shopImg']);
	        	//建立店铺配置信息
	        	$sc = [];
	        	$sc['shopId'] = $shopId;
	...
	public function edit(){
        ...
        try{
	        ...
	        $areaIds = model('Areas')->getParentIs($data['areaId']);
		    if(!empty($areaIds))$data['areaIdPath'] = implode('_',$areaIds)."_";
	        FIUnset($data,'shopId,userId,dataFlag,createTime,applyId,goodsCatIds,accredIds,isSelf');
	        //启用上传图片
			FIUseImages(1, $shopId, $data['shopImg'],'shops','shopImg');
