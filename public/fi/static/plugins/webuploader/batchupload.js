...
function batchUpload(options){
    ...
    if ( !WebUploader.Uploader.support() ) {
        FI.msg( 'Web Uploader 不支持您的浏览器！如果你使用的是IE浏览器，请尝试升级 flash 播放器',{icon:2});
    ...
    uploader = WebUploader.create({
        ...
        // swf文件路径
        swf: FI.STATIC + '/plugins/webuploader/js/Uploader.swf',
    ...
    uploader.onError = function( code ) {
    	if(code=='Q_EXCEED_NUM_LIMIT'){
    		FI.msg( '文件上传数量超过限制' ,{icon:2});
    	}else if(code == 'F_EXCEED_SIZE'){
            FI.msg( '上传的文件大小超过限制' ,{icon:2});
    	}else if(code == 'Q_TYPE_DENIED'){
    		FI.msg( '上传的文件类型错误' ,{icon:2});
    	}else{
    		FI.msg( 'Eroor: ' + code ,{icon:2});
    	}
    };
