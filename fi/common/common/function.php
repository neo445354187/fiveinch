...
function FISendMail($to, $subject, $content) {
    ...
    $mail->AddAddress($to);
    $mail->FromName = FIConf("CONF.mailSendTitle");
    $mail->IsHTML(true);
    // 装配邮件正文信息
...
function FICheckLoginKey($val, $userId = 0) {
    ...
    $rs = $dbo->count();
    if ($rs == 0) {
        return FIReturn("该登录账号可用", 1);
    }
    return FIReturn("对不起，登录账号已存在");
}
...
function FIImg($imgurl, $imgType = 1) {
    ...
    return ((file_exists(FIRootPath() . "/" . $img)) ? $img : $imgurl);
}
